class DrawLine {
            ...
            `<div id="line` + _this.opts.modelNum + _this.numIndex + `" class="card-label card-line` + _this.opts.modelNum + `">
                <span class="text">
                    ...
                    <span> 
                        ...
                        <img id="linePoints` + _this.opts.modelNum + _this.numIndex + `"  class="linePoint" src='/guide/ScenePreview/v0.1.6/assets/img/measure/remove.png'>
                    </span>
                </span>
            </div>`;
        ...
        _this.opts.app.on('SingleClick', function(e) {
            if (e.button == 0) {
                ...
                if (_this.lineDistanceAll != null) {
                    ...
                    _this.lineDistanceAll = _this.lineDistanceAll/1000;
                    _this.lineDistanceAll = _this.lineDistanceAll.toFixed(2);
                    _this.template += _this.lineDistanceAll + `公里`;
                } else {
                    _this.template += `<span style="color:#f45905; border-right: 1px solid #ccc;margin-right: 5px">` + _this.lineId + `</span> 起点`
                }
                _this.template +=
                    `</span>
                    <span><img id="linePoints` + _this.opts.modelNum + _this.numIndex + `" class="linePoint" src='/guide/ScenePreview/v0.1.6/assets/img/measure/remove.png'></span> 
                    </div>`;
                _this.boardId = 'line' + _this.opts.modelNum + _this.numIndex;
                _this.createCard(_this.regionPoint);
                _this.pointsObj = {
                ...
                _this.end();
                _this.opts.app.off(THING.EventType.SingleClick, '*', '创建测距线');
                $("#distance").removeClass("distanceactive")
            }
            _this.rianleyDom.css('display', 'none');
    ...
    changeText() {
        ...
        _this.countNum = _this.countNum/1000;
        _this.countNum = _this.countNum.toFixed(2);
        // $('#' + nodeId + ' .text').text('共' + _this.countNum + ' 米');
        $('#' + nodeId + ' .text').text('共' + _this.countNum + ' 公里');
        if (!_this.lastStatus) {
            $('#' + nodeId).append(`<span><img class="destory" src='/guide/ScenePreview/v0.1.6/assets/img/measure/destroy.png'></span>`);
    ...
    panelText() {
        ...
        this.pointsObjArr.forEach((v, k) => {
            let a = v.coor.map(val => val.toFixed(3));//sence
            let b = CMAP.Util.convertWorldToLonlat(a);//city
            b.pop();//city
            let lonlatarr = [];//city
            b.forEach(e =>{
                let c;
     ...
     */
        ...
        _this.createPanel();
        _this.panelText();
        let p = _this.opts.app.query('linePoints' + _this.opts.modelNum);
        // 为节点绑定鼠标按下事件开启拖拽
    ...
    pointEvent() {
        var _this = this;
        let p = _this.opts.app.query('linePoints' + this.opts.modelNum);
    ...
    changeVal() {
        ...
        this.pointsObjArr.forEach((v, k) => {
            if (k > 0 && k < this.pointsObjArr.length - 1) {
                ...
                count = count/1000;//米改为公里
                count = count.toFixed(2);
                let nodeId = v.parent;
                $('#' + nodeId + ' .text').text(count + '公里');
            }
        })
    }
